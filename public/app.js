...
function fitHtml(d, psi, places){
  ...
  const onOtherBuilder = platform.some(p => /wix|squarespace|weebly|duda/i.test(p));
  ...
  const COPY = {
    design: {
      ...
      migrate: 'Because the site is on a third-party builder, a redesign would also move it onto WordPress, which is easier to extend and to optimize later.'
    },
    seo: {
      title: 'SEO Services',
