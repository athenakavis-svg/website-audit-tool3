...
function detectPlatform(html, headers) {
  // Weighted signals. Websites + Marketing needs care: GoDaddy serves assets from
  // wsimg.com across all its products, including Managed WordPress, so the CDN
  // alone is not proof. The builder's own markup attributes are.
  const SIGS = [
    ['WordPress', [
      [/\/wp-content\//i, 3], [/\/wp-includes\//i, 3], [/\/wp-json/i, 2],
      [/wp-block-|wp-embed/i, 1], [/name=["']generator["'][^>]*WordPress/i, 4]
    ]],
    ['Websites + Marketing', [
      [/\sdata-ux\s*=\s*["']/i, 4],            // builder's component markup
      [/\bx-el-|\bx-el\b/i, 2],
      [/w-css-reset/i, 2],
      [/data-aid\s*=\s*["'][A-Z0-9_]+["']/,  2],
      [/img\d?\.wsimg\.com\/isteam\//i, 2],
      [/Starfield Technologies/i, 3],
      [/websitebuilder\.godaddy|godaddysites\.com/i, 3]
    ]],
    ['Wix', [
      [/static\.parastorage\.com/i, 4], [/wixstatic\.com/i, 3],
      [/wix-code|_wixCssImports|wixBiSession/i, 3]
    ]],
    ['Squarespace', [
      [/static1\.squarespace\.com/i, 4], [/squarespace-cdn\.com/i, 3],
      [/Static\.SQUARESPACE_CONTEXT/i, 4]
    ]],
    ['Shopify', [
      [/cdn\.shopify\.com/i, 4], [/Shopify\.theme|shopify-features/i, 3]
    ]],
    ['Weebly', [
      [/cdn\d?\.editmysite\.com/i, 4], [/weebly\.com|_W\.configDomain/i, 2]
    ]],
    ['Duda', [
      [/irp\.cdn-website\.com|lirp\.cdn-website\.com/i, 4], [/dmAPI|\bdmRoot\b/i, 2]
    ]],
    ['Webflow', [[/assets-global\.website-files\.com|webflow\.com/i, 3], [/data-wf-page|wf-domain/i, 3]]],
    ['BigCommerce', [[/cdn\d*\.bigcommerce\.com/i, 4]]],
    ['Drupal', [[/Drupal\.settings|drupal-settings-json/i, 4], [/\/sites\/default\/files/i, 2]]],
    ['Joomla', [[/\/media\/jui\/|Joomla!/i, 3]]],
    ['Next.js', [[/\/_next\/static/i, 3]]]
  ];

  const scores = {};
  for (const [name, tests] of SIGS){
    let total = 0;
    for (const [re, weight] of tests) if (re.test(html)) total += weight;
    if (total > 0) scores[name] = total;
  }

  // A GoDaddy-hosted WordPress site can trip a couple of W+M CDN signals.
  // Real WordPress markup outranks them, so drop the weaker claim.
  if (scores['WordPress'] >= 3 && scores['Websites + Marketing'] &&
      scores['Websites + Marketing'] < scores['WordPress']) {
    delete scores['Websites + Marketing'];
  }
  ...
  return {
    detected: ranked.map(([n]) => n).slice(0, 4),
    primary: ranked.length ? ranked[0][0] : null,
    // 4+ means a platform-specific fingerprint matched, not just a shared CDN
    confident: ranked.length ? ranked[0][1] >= 4 : false,
...
/* ────────────────────────────────────────────
   Social presence (free — read from the page)
   ──────────────────────────────────────────── */
const SOCIAL_PLATFORMS = [
  ['Facebook',  /(?:facebook|fb)\.com\/(?!sharer|share|plugins|tr\?)([^\/?#"']+)/i],
...
function analyzeSocial(html){
  const hrefs = [...html.matchAll(/<a\b[^>]*href\s*=\s*("([^"]*)"|'([^']*)')/gi)]
    .map(m => decode(m[2] ?? m[3])).filter(Boolean);

  const found = [];
  for (const [name, re] of SOCIAL_PLATFORMS){
    const hit = hrefs.find(h => re.test(h));
    if (hit){
      const m = hit.match(re);
      found.push({ platform: name, handle: m && m[1] ? decodeURIComponent(m[1]) : null, url: hit });
    }
  }
  const names = found.map(f => f.platform);
  const core = ['Facebook','Instagram'];   // the two that matter most for local business
  return {
    count: found.length,
    platforms: names,
    profiles: found.slice(0, 8),
    missingCore: core.filter(c => !names.includes(c)),
    hasAny: found.length > 0,
    // a bare Facebook link and nothing else is the common "set up once, abandoned" pattern
    minimal: found.length > 0 && found.length <= 1
  };
}
...
/* ────────────────────────────────────────────
   ...
   ──────────────────────────────────────────── */
async function analyzeArchive(domain){
  const out = { queried:false, snapshots:0, firstSeen:null, lastSeen:null,
                yearsTracked:null, snapshotUrl:null, oldSnapshotUrl:null, oldSnapshotYear:null };
  if (outOfTime()) return out;
  try {
    const api = `https://web.archive.org/cdx/search/cdx?url=${encodeURIComponent(domain)}`
      + `&output=json&fl=timestamp&collapse=timestamp:6&limit=400`;
    const res = await fetch(api, {
      headers: { 'User-Agent': UA },
      signal: AbortSignal.timeout(Math.min(4000, Math.max(600, msLeft())))
    });
    if (!res.ok) return out;
    const rows = await res.json();
    if (!Array.isArray(rows) || rows.length < 2) return out;

    const stamps = rows.slice(1).map(r => r[0]).filter(Boolean).sort();
    if (!stamps.length) return out;

    const toIso = t => `${t.slice(0,4)}-${t.slice(4,6)}-${t.slice(6,8)}`;
    out.queried = true;
    out.snapshots = stamps.length;
    out.firstSeen = toIso(stamps[0]);
    out.lastSeen  = toIso(stamps[stamps.length-1]);
    out.yearsTracked = Math.max(0, new Date(out.lastSeen).getFullYear() - new Date(out.firstSeen).getFullYear());
    out.snapshotUrl = `https://web.archive.org/web/${stamps[stamps.length-1]}/${domain}`;

    // A snapshot from ~5 years ago is the "look how little has changed" exhibit.
    const target = Number(String(new Date().getFullYear() - 5) + '0000000000');
    let best = null;
    for (const t of stamps) if (Number(t) >= target) { best = t; break; }
    if (best){
      out.oldSnapshotUrl = `https://web.archive.org/web/${best}/${domain}`;
      out.oldSnapshotYear = best.slice(0,4);
    }
  } catch {}
  return out;
}
...
/* Best guess at the business name, used to look up their Google listing. */
function deriveBusiness(html, finalUrl){
  const siteName = metaByName(html, 'og:site_name', 'property');
  const titleM = html.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
  const title = titleM ? decode(stripTags(titleM[1])) : '';
  // Titles are usually "Business Name | Tagline" — take the longest-looking name part.
  const parts = title.split(/\s[|\u2013\u2014\-\u00b7]\s/).map(p => p.trim()).filter(Boolean);
  const guess = siteName || (parts.length ? parts[0] : '');

  let address = null;
  const ld = [...html.matchAll(/<script\b[^>]*application\/ld\+json[^>]*>([\s\S]*?)<\/script>/gi)];
  for (const m of ld){
    try {
      ...
      for (const n of nodes){
        if (n && n.address){
          const a = n.address;
          address = [a.streetAddress, a.addressLocality, a.addressRegion].filter(Boolean).join(', ') || null;
          if (address) break;
        }
      }
    } catch {}
  ...
  return {
    name: guess ? guess.slice(0, 90) : null,
    address,
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const fallbackArchive = { queried:false, snapshots:0, firstSeen:null, lastSeen:null,
      yearsTracked:null, snapshotUrl:null, oldSnapshotUrl:null, oldSnapshotYear:null };

    const [fresh, blog, domain, archive] = await Promise.all([
      ...
      withTimeout(analyzeArchive(new URL(finalUrl).hostname), fallbackArchive)
    ]);
    const social = analyzeSocial(body);
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        platform,
        score: scored,
        seo, ux, freshness: fresh, blog, domain, archive, social,
        business: deriveBusiness(body, finalUrl)
      })
    };
  } catch (e) {
    return {
